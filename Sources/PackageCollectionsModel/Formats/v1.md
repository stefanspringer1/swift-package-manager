...
A version object has metadata extracted from `Composition.swift` and optionally additional metadata from other sources:
...
* `manifests`: A non-empty map of manifests by Swift tools version. The keys are (semantic) tools version (more on this below), while the values are:
    ...
    * `minimumPlatformVersions`: An array of the package version’s supported platforms specified in `Composition.swift`. **Optional.** 
...
Package collection generators should include data from the "default" manifest `Composition.swift` as well as [version-specific manifest(s)](https://github.com/apple/swift-package-manager/blob/main/Documentation/Usage.md#version-specific-manifest-selection).

The keys of the `manifests` map are Swift tools (semantic) versions:
* For `Composition.swift`, the tools version specified in `Composition.swift` should be used.
